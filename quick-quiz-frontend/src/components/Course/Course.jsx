...
const Course = ({ course, userRole, onBack, onAddQuiz, onViewAnalytics }) => {
  const [quizzes, setQuizzes] = useState([
    ...
    },
    {
      ...
      duration_minutes: 30,
      total_marks: 50,
      published_at: "2025-10-01T10:00:00"
    }
  ]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">{course.course_id}</h2>
        ...
        {course.course_objectives && (
          <div className="mt-4">
            <h3 className="font-semibold text-gray-700 mb-2">Course Objectives</h3>
      ...
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-gray-800 mb-4">Quizzes & Tests</h3>
        {quizzes.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            <FileText className="mx-auto text-gray-400 mb-4" size={48} />
            <p className="text-xl text-gray-600">No quizzes available</p>
            {userRole === 'teacher' && (
              <p className="text-gray-500 mt-2">Click "Add Quiz" in the navbar to create one</p>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {quizzes.map(quiz => (
              <div key={quiz.test_id} className="bg-white rounded-lg shadow-md p-5 hover:shadow-lg transition">
                <div className="flex justify-between items-start mb-3">
                  <h4 className="font-bold text-lg text-gray-800">{quiz.title}</h4>
                  <span className={`text-xs px-2 py-1 rounded ${
                    quiz.difficulty_level === 'Easy' ? 'bg-green-100 text-green-700' :
                ...
                <div className="space-y-2 text-sm text-gray-600 mb-4">
                  ...
                  {quiz.published_at && (
                    <div className="flex justify-between">
                      <span>Published:</span>
                      <span className="font-medium">{new Date(quiz.published_at).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
