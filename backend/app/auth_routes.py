...
def user_granted_admin(email_id):
    allowed_admins = [
        ...
        "aniketw@cse.iitb.ac.in",
        "parikhninad@cse.iitb.ac.in",
    ]
...
def auth_register():
    ...
    if not validate_password(password):
        return (
            jsonify(
                {
                    "error": "Password must have 8-16 characters, with at least 1 of special character, number and capital letter"
                }
            ),
            400,
        )
    ...
    if not validate_role(role):
        return (
            jsonify({"error": "Role can only be one of teacher, student or admin"}),
            400,
        )
