            ...
            courses.append(
                {
                    ...
                    "enrolled_at": (
                        enrollment.taken_at.isoformat() if enrollment.taken_at else None
                    ),
                }
            )
...
def list_quizzes_for_student(course_id):
    ...
    for test in tests:
        submitted_attempt = StudentTestAttempt.query.filter_by(
            test_id=test.test_id, student_id=user.id, status="submitted"
        ).first()

        in_progress_attempt = StudentTestAttempt.query.filter_by(
            test_id=test.test_id, student_id=user.id, status="in_progress"
        ).first()
        ...
        result.append(
            {
                ...
                "attempt_id": (
                    in_progress_attempt.attempt_id if in_progress_attempt else None
                ),
            }
        )
...
def list_questions_for_quiz(test_id):
    ...
    if not attempt:
        return (
            jsonify({"error": "No active attempt found. Start an attempt first."}),
            403,
        )
    ...
    saved_answers = {
        ...
        for qa in StudentQuestionAttempt.query.filter_by(
            attempt_id=attempt.attempt_id
        ).all()
    }
...
def student_results():
    ...
    for test_id, atts in test_attempts.items():
        ...
        if submitted_attempts:
            ...
            last_attempt = max(
                submitted_attempts, key=lambda a: a.submitted_at or datetime.min
            )
            last_score = last_attempt.total_score
            last_attempted_at = (
                last_attempt.submitted_at.isoformat()
        ...
        results.append(
            {
                ...
                "passed": (
                    best_score >= test.passing_marks if best_score is not None else None
                ),
            }
        )
    ...
    for attempt in attempts:
        ...
        for qa in question_attempts:
            ...
            qa_dict = {
                ...
                "selected_answer": (
                    json.loads(qa.selected_answer) if qa.selected_answer else None
                ),
            ...
            if attempt.status == "submitted":
                qa_dict["correct_answer"] = json.loads(question.correct_answer)
                qa_dict["options"] = (
                    json.loads(question.options) if question.options else None
                )
...
def student_analytics(test_id):
    ...
    analytics = {
        ...
        "average_percentage": sum(a.percentage for a in attempts if a.percentage)
        / len(attempts),
        "pass_count": sum(1 for a in attempts if a.passed),
        "average_time_seconds": sum(
            a.time_taken_seconds for a in attempts if a.time_taken_seconds
        )
        / len(attempts),
        "topic_performance": {},
        ...
        try:
            ...
            for tag in tags:
                if tag not in stats["tags"]:
                    stats["tags"][tag] = [0, 0]
                stats["tags"][tag][0] += marks_obtained
                stats["tags"][tag][1] += total_marks
        except:
            pass
